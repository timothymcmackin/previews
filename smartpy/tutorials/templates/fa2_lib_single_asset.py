...
def my_module():
    ...
    class MySingleAssetContract(
        ...
        def __init__(self, admin_address, contract_metadata, ledger, token_metadata):
            """Initializes the contract with administrative permissions and single asset functionalities.
            The base class is required; all mixins are optional.
            The initialization must follow this order:

            - Other mixins such as OnchainviewBalanceOf, MintSingleAsset, and BurnSingleAsset
            - Base class: SingleAsset
            - Transfer policy
            - Admin
            """
            ...
            # Initialize the single asset token-specific entrypoints
            main.MintSingleAsset.__init__(self)
            main.BurnSingleAsset.__init__(self)

            # Initialize single asset base class
...
def test():
    ...
    scenario = sp.test_scenario("fa2_lib_single_asset", my_module)
    scenario.h1("FA2 single asset contract test")
    ...
    # Instantiate the FA2 single asset contract
    contract = my_module.MySingleAssetContract(
        admin.address, sp.big_map(), {}, token_metadata
    )

    # Build contract metadata content
    contract_metadata = sp.create_tzip16_metadata(
        name="My FA2 single asset contract",
        description="This is an FA2 single asset contract using SmartPy.",
        version="1.0.0",
        ...
        homepage="https://smartpy.io/ide?template=fa2_lib_single_asset.py",
    ...
    scenario.verify(
        _get_balance(contract, sp.record(owner=bob.address, token_id=0)) == 100
    )
    ...
    scenario.verify(_total_supply(contract, sp.record(token_id=0)) == 400)
    ...
    scenario.verify(
        _get_balance(contract, sp.record(owner=bob.address, token_id=0)) == 205
    )
    ...
    contract.transfer(
        [
            sp.record(
                from_=bob.address,
                txs=[sp.record(to_=alice.address, amount=10, token_id=0)],
            ),
        ],
        _sender=bob,
    )
    # Verify the result
    scenario.verify(
        _get_balance(contract, sp.record(owner=alice.address, token_id=0)) == 205
    )
    scenario.verify(
        _get_balance(contract, sp.record(owner=bob.address, token_id=0)) == 195
    )
    ...
    scenario.verify(
        _get_balance(contract, sp.record(owner=alice.address, token_id=0)) == 204
    )
    # You can't burn someone else's tokens
    contract.burn(
