...
``1/100 * BLOCKS_PER_CYCLE * CONSENSUS_COMMITTEE_SIZE = 1/100 * 1800 * 7000 = 126,000``
slots. Assume also that the attesting power of C's attestations
included during that cycle has been ``91,456`` slots. Given that this number is
bigger than the minimum required (``126,000 * 2 / 3``), it receives an attesting
reward of ``126,000 * 0.000761 = 95.886`` tez for that cycle.
...
.. _slashing_recovery_alpha:

Recovering from being slashed
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

When a baker is slashed, they are :ref:`forbidden<new_forbidden_period_alpha>` from both baking and attesting for a certain number of cycles.
No manual action is required to recover from being slashed; bakers' rights are reinstated automatically after a few cycles.
However, the effect of the penalty, including the cycles in which the baker can and cannot bake, is unintuitive.
In fact, the effect of the forbidden period means that the slashed baker becomes inactive, active, inactive again, and active again over the next few cycles.
The rest of this section is a detailed explanation of the effect of slashing on a baker's rights and what they can do to reduce that effect.

Slashing can affect a baker's rights for several cycles.
For example, let's assume this timeline of a slashed baker, which assumes that ``CONSENSUS_RIGHTS_DELAY`` is 2:

#. Cycle ``n``: A baker double signs.
   The baker is immediately denounced for double-signing.
   As a result, the baker is forbidden from signing for the remainder of the cycle.
#. Cycle ``n + 1``: The baker is still forbidden as part of the penalty.
   At the end of this cycle, the baker is slashed, which reduces its stake.
#. Cycle ``n + 2``: The penalty for double-signing is over at the beginning of this cycle.
   However, the baker is still forbidden because their rights for this cycle were calculated based on their stake at the end of cycle ``n - 1``.
   Because the slashing penalty reduced its stake, the baker's current stake is less than the stake that was used to calculate their rights for this cycle.
   Thus the baker is forbidden for the cycle.
#. Cycle ``n + 3``: Similar to the previous cycle, the baker has no rights because their current stake is lower than the stake that was used to calculate their rights for this cycle.
#. Cycle ``n + 4``: The baker becomes active again because its current stake matches the stake that was used to calculate rights for this cycle at the end of cycle ``n + 1``, which includes the deduction from the slashing penalty.
#. Cycle ``n + 5``: The baker becomes inactive again because they were inactive in cycle ``n + 2`` and therefore have no rights in this cycle.
#. Cycle ``n + 6``: Similar to the previous cycle, the baker is inactive because they were inactive in cycle ``n + 3`` and therefore have no rights in this cycle.
#. Cycle ``n + 7``: The baker becomes active again with rights based on their stake at the end of cycle ``n + 4``.

In this way, being slashed can stop a baker from baking for five full cycles plus part of the cycle in which they were denounced.

.. figure:: slashing-recovery-a.png

 Figure 1. A timeline of the effect of slashing on a baker's rights and stake

Bakers can reduce the time that they lose baking rights by staking more tez to make up for the slashing penalty.
For example, let's assume this similar timeline of a slashed baker who replaces the slashed tez with newly staked tez:
...
#. Cycle ``n + 2``: The penalty for double-signing is over at the beginning of this cycle.
   ...
   Unlike the previous timeline, the baker adds more tez to make up for the slashed amount before the end of cycle ``n + 2``.

#. Cycle ``n + 3``: Unlike the previous timeline, the baker bas baking rights because the tez they added in cycle ``n + 2`` makes up for the slashed tez.
   Because they added tez, their current stake is at least as large as the stake that their rights were calculated on at the end of cycle ``n + 2``.
#. Cycle ``n + 4``: Similar to the previous cycle, the baker has baking rights.
#. Cycle ``n + 5``: The baker becomes inactive again because they were inactive in cycle ``n + 2`` and therefore have no rights in this cycle.
#. Cycle ``n + 6``: The baker becomes active again with rights based on their stake at the end of cycle ``n + 3``.

In this timeline, adding tez to replace the slashing penalty reduced the number of cycles that the baker was inactive by two.

.. figure:: slashing-recovery-b.png

 Figure 1. A timeline of the effect of slashing on a baker's rights and stake when the baker replaces slashed tez
