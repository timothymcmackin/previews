...
.. _slashing_recovery_seoul:

Recovering from being slashed
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

When a baker is slashed, they are :ref:`forbidden<new_forbidden_period_seoul>` from both baking and attesting for a certain number of cycles.
